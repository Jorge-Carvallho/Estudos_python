...
print('-------------------------------------Boas vindas personalizada--------------------------------------')



# Função de Boas-Vindas Personalizada
# Implemente uma função boas_vindas que recebe uma saudação e vários nomes como *args. Use **kwargs para receber informações opcionais, como emoji e pontuacao, para customizar a saudação. Retorne todas as saudações formatadas em linhas separadas.
def boas_vindas(saudacao,*args,**kwargs):
    emoji = kwargs.get('emoji', '')
    pontuacao = kwargs.get('pontuacao', '!')
    ...
    for nome in args:
        saudacao_individual = f'{saudacao}, {nome}{emoji}{pontuacao}'
        saudacao_formatadas.append(saudacao_individual)

    return '\n'.join(saudacao_formatadas)


print(boas_vindas("Olá", "Ana", "Carlos", "João", emoji="😊", pontuacao="!!"))
...
# Crie uma função lambda que receba uma idade e retorne "Maior de idade" se a idade for 18 ou mais, e "Menor de idade" caso contrário.

# Dobrar Todos os Números Ímpares de uma Lista
# Crie uma função lambda que receba uma lista de números e retorne uma nova lista com apenas os números ímpares dobrados.

# Filtrar Strings de Comprimento Par
# Crie uma função lambda que receba uma lista de strings e retorne uma nova lista apenas com as strings de comprimento par.